...
def get_args():
    ...
    parser.add_argument('--exp', default=7, type=int)
...
if __name__ == '__main__':
    args = get_args()
    args.out = f"{args.result_folder}/results/{args.exp}_{args.task_name}/evaluate/{args.exp}_{args.task_name}_{args.prompt_idx}_{args.out_num}.txt"
    ...
    with open(f'{args.result_folder}/results/{args.exp}_{args.task_name}/{args.exp}_test_attr.json', 'r') as json_file:
    ...
    prompt = open(f'prompts/{args.task_name}_multi.md').read()
    with open(args.out, 'a') as outf:
        outf.write(f'prompt-------------------------\n')
        outf.write(f'{prompt}\n\n')
    ...
    if args.attributes:
        if args.generate:
            ...
            attrs = task.get_attr(args.mode, generate_prompt, exs, exp=args.exp)
    else:
        attrs = None
