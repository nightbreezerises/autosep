...
每个数据集需要一个 `split_datasets_images.json` 文件（优先）或 `split_dataset_images.json` 文件，格式如下：
...
└── dogs_120/
    ...
    │   └── ...
    └── images_split/
        └── split_datasets_images.json       # 划分文件
```
...
**重要**: 划分文件中的路径是相对于数据集根目录的，例如：
- dogs_120: `"Images/n02088094-Afghan_hound/xxx.jpg"`
- flowers_102: `"jpg/image_06992.jpg"`
- CUB_200_2011: `"CUB_200_2011/images/001.xxx/xxx.jpg"`
...
**重要**: 图片路径使用**相对路径**，相对于数据集根目录（`images_split` 所在目录）。

```
工作目录: /home/hdl/project/autosep_new
数据目录: ./datasets (相对于工作目录)

划分文件位置（优先查找顺序）:
1. ./datasets/{dataset_name}/images_split/split_datasets_images.json
2. ./datasets/{dataset_name}/images_split/split_dataset_images.json
3. ./datasets/{dataset_name}/{dataset_name}/images_split/split_datasets_images.json
4. ./datasets/{dataset_name}/{dataset_name}/images_split/split_dataset_images.json
...
实际完整路径:
./datasets/dogs_120/Images/n02088094-Afghan_hound/xxx.jpg
./datasets/flowers_102/jpg/image_06992.jpg
./datasets/CUB_200_2011/CUB_200_2011/images/001.xxx/xxx.jpg
```
...
1. **创建数据集目录**
   ```bash
   # 单层结构（推荐）
   mkdir -p datasets/your_dataset/images
   mkdir -p datasets/your_dataset/images_split
...
3. **创建划分文件** `split_datasets_images.json`
   ```json
   {
     "train": [
   ...
   **字段说明**:
   - 第1个元素: 类别名称 (字符串)
   - 第2个元素: 类别ID (整数，从0开始)
   - 第3个元素: 图片相对路径 (相对于 `images_split` 所在目录)
...
**问题描述**：
```
MLLMBot inference error: 'Qwen2_5_VLTextModel' object has no attribute 'generate'
MLLMBot inference error: 'Qwen3VLTextModel' object has no attribute 'generate'
```

**问题原因**：
`call_llm` 方法中错误地调用了 `self.model.language_model.generate()`：
- `Qwen2_5_VLForConditionalGeneration.language_model` 返回的是 `Qwen2_5_VLTextModel`
- `Qwen3VLForConditionalGeneration.language_model` 返回的是 `Qwen3VLTextModel`
- 这些 `TextModel` 类**没有** `generate` 方法，只有完整的 `ForConditionalGeneration` 模型才有

**解决方案**：
修改 `agents/mllm_bot_qwen_2_5_vl.py` 和 `agents/mllm_bot_qwen_3_vl.py` 中的 `call_llm` 方法，使用完整模型的 `generate` 方法：
...
**涉及文件**：
- `agents/mllm_bot_qwen_2_5_vl.py` - `call_llm` 方法
- `agents/mllm_bot_qwen_3_vl.py` - `call_llm` 方法

## 七、脚本使用指南

详见 `scripts/脚本使用指南.md`，包含：
- 脚本执行流程图
- 配置文件详细说明
- 后台运行和日志管理
- 常见问题解答
