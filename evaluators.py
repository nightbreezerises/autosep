...
class SuccessiveHalvingEvaluator:
    ...
    def __call__(self, prompts, exs, predictor, scorer, pred_prompts=None, attribute_cache=None, rounds=40,
    ...
    def __call__(self, prompts, exs, predictor, scorer, pred_prompts=None, attribute_cache=None, rounds=40,
        ...
        for ri in tqdm(range(rounds), desc=f'Evaluating {len(prompts)} prompts'):
            ...
            while num_attempts < 10:
                num_attempts += 1
                try:
                    scores = scorer(predictor, sampled_prompts, sampled_data, pred_prompts, attribute_cache,
...
class BruteForceEvaluator:
    ...
    def __call__(self, prompts, exs, predictor, scorer, pred_prompts=None, attribute_cache=None, rounds=40,
        ...
        sample_size = min(len(exs), int(self.config['eval_budget']))
        ...
        while num_attempts < 10:
            num_attempts += 1
            try:
                scores = scorer(predictor, prompts, eval_exs, pred_prompts, attribute_cache, max_threads=max_threads,
...
class LLMEvaluator:
    ...
    def __call__(self, prompts, scorer):
        while True:
            try:
